...
> **참고**: 아래 스크린샷은 외부 평가 피드백을 반영해 성능 표에 표준편차 컬럼을 추가하고,
> 입력 오류 메시지에 줄 번호를 표시하도록 개선하기 **이전** 버전으로 캡처한 것이다.
> 판정 로직·PASS/FAIL 결과·MAC 점수는 이후 버전과 완전히 동일하며(코드 로직 자체는
> 바뀌지 않음), 성능 표 컬럼 구성과 오류 메시지 문구만 지금 코드와 미세하게 다르다.
> 정확한 최신 출력 형식은 "결과 리포트 4), 5)"의 텍스트 예시를 참고할 것.
...
- **우선 적용할 최적화 방향** (구현 우선순위 순):
    1. **2차원 → 1차원 배열 변환** (이 과제의 보너스 과제이기도 함): `matrix[i][j]` 대신
       `flat[i*n+j]`로 접근하면 중첩 리스트의 이중 인덱싱 오버헤드가 줄어들고, 메모리
    ...
    3. **병렬화**: `multiprocessing`으로 행 단위 작업을 여러 프로세스에 나누거나, 표준
       ...
       "왜 NPU가 필요한가"를 보여주려던 지점이 바로 여기다.

현재 코드는 과제 요구사항(외부 라이브러리 금지, 3x3~25x25 지원)을 만족시키는 범위에서
가장 단순하고 읽기 쉬운 구현을 택했고, 위 최적화들은 이 범위를 벗어나는 "더 큰 입력을
