...
- `main.py`는 실행 시 **자기 자신의 위치를 기준**으로 `data.json`을 찾습니다
  ...
  무관하게, 위 3개 파일이 **같은 폴더**에 있기만 하면 정상 동작합니다.
- 별도의 하위 폴더(`src/`, `data/` 등)로 나누지 않고 3개 파일을 한 폴더에 두는 평면
  구조를 사용했습니다. 표준 라이브러리만 쓰는 단일 스크립트 규모라 패키지 분리가
  불필요하다고 판단했습니다.
