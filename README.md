...
└── docs/
    └── screenshots/     ← README에 삽입된 실제 실행 결과 스크린샷 (11장)
...
8. **점수 출력 형식**: MAC 점수는 파이썬 `float` 기본 표현 그대로 출력하고 일부러
   반올림하지 않습니다. `0.9`와 `0.8999999999999999`처럼 부동소수점 오차를 있는
   그대로 눈으로 볼 수 있어야 "왜 epsilon 정책이 필요한지" 체감할 수 있기 때문입니다.
...
**덧붙여, 모드1과 모드2가 같은 "동점" 상태를 다르게 표기하는 이유도 같은 맥락에서
설명할 수 있다.** 모드1은 "판정 불가"라는 한국어 문장을, 모드2는 "UNDECIDED"라는
영문 상태값을 쓰는데, 판정 로직(epsilon 비교) 자체는 완전히 동일하고 표기만 다르다.
모드1은 필터를 "A", "B"라는 임의의 이름으로 다루기 때문에 사람이 읽을 문장으로 직접
설명하는 게 자연스럽다. 반면 모드2는 `"Cross"`, `"X"`라는 프로그램 내부 표준 라벨
체계를 쓰고 있어서, 여기에 "이도 저도 아님"을 나타내는 세 번째 상태도 같은 형식
(영문 대문자)의 전용 상태값 `"UNDECIDED"`로 맞춰, `judge == expected_label` 같은
문자열 비교가 항상 일관되게 동작하도록 했다. 즉 모드1은 "사람이 읽을 문장"이 목적이고
모드2는 "표준 라벨 체계 안에서 프로그램이 비교하기 쉬운 상태값"이 목적이라 표기를
분리한 것이다.
...
이처럼 필터마다 값의 범위(스케일)가 다를 수 있다는 걸 알면서도, 이 프로젝트는 점수를
별도로 정규화(스케일 조정)하지 않고 원본 값 그대로 MAC 연산에 사용합니다. 이렇게 해도
문제가 없는 이유는, 판정이 "서로 다른 필터끼리 점수를 직접 비교"하는 게 아니라 "같은
패턴에 대해 Cross 필터 점수와 X 필터 점수"만 비교하기 때문입니다. 즉 비교 대상 두
점수는 항상 같은 패턴, 같은 스케일의 필터 집합 안에서 나온 값이라 스케일 차이가
판정 결과를 왜곡하지 않습니다.
...
### 5) 대규모 입력(예: 1000x1000)으로 확장하면 어떻게 될까

현재 구현은 3x3–25x25 수준에서는 충분히 빠르지만(위 표 참고, 최대 수십 ms), 이 구조
그대로 훨씬 더 큰 입력을 처리하면 어떤 문제가 생길지 실제로 확인해봤다. (아래 수치는
추정이 아니라, 실제로 `generate_cross(1000)`으로 1000x1000 십자가 패턴을 만들어
`mac_operation()`을 3회 직접 실행하고 측정한 값이다.)
...
- **실측 결과 (N=1000, 3회 측정)**:
  ...
  횟수 비율과 같은 자릿수로 증가하는 것을 확인했다(정확히 비례하지 않는 이유는 위
  "실측치가 이론(N²)과 완전히 비례하지 않는 이유"와 동일하게 캐시/인터프리터
  오버헤드 때문이다). 절대적으로는 100ms 안팎이라 여전히 감당 가능한 수준이지만,
  필터가 "수백×수백 크기를 수천 개" 쓰는 실제 AI 모델 규모로 가면(이 README 도입부에서
  언급한 것처럼) 수억 번의 MAC 연산이 필요해지고, 이 순수 파이썬 반복문 구조로는
  감당하기 어려워진다.
...
- **우선 적용할 최적화 방향** (구현 우선순위 순):
  1. **2차원 → 1차원 배열 변환** — ✅ **실제로 구현하고 실측까지 완료했다** (아래
     ...
     25x25 이하에서는 약 45–66%, 1000x1000에서는 약 59.7% 더 빨라졌다. 다만 이건
     상수 배율 개선이라, N이 계속 커지면 이것만으로는 부족하고 아래 2, 3번이 추가로
     필요하다.
  ...
  3. **병렬화**: 예를 들어 `multiprocessing.Pool`로 1000개 행을 4개 프로세스에
     ...
     크기에서는 그 오버헤드가 이득을 상쇄할 수도 있다(작업 자체가 100ms 안팎으로
     이미 짧기 때문). 워커 간 통신 비용보다 각 워커의 작업량이 충분히 커야(예:
     10000x10000 이상) 병렬화의 이득이 뚜렷해질 것으로 예상된다. 표준 라이브러리
     범위를 벗어나도 된다면 NumPy 같은 벡터화 라이브러리, 더 나아가 실제 GPU/NPU
     같은 전용 병렬 하드웨어를 쓰는 것이 근본적인 해결책이다. 이 프로젝트가 "왜
     NPU가 필요한가"를 보여주려던 지점이 바로 여기다. (이번 제출 범위에는 미구현)

  현재 코드는 과제 요구사항(외부 라이브러리 금지, 3x3–25x25 지원)을 만족시키는
  범위에서 1번(1차원 배열 최적화)까지는 실제로 구현했고, 2·3번은 "더 큰 입력을
  지원하려면"이라는 가정 하의 확장 방향으로 남겨두었다.

### 6) 콘솔 요약 (data.json 실행 결과)
